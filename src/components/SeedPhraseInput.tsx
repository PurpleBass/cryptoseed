  ...
  return <div className="space-y-4">
      ...
      <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <Button type="button" variant="outline" size="sm" onClick={clearWalletMetadata} className="text-gray-500 border-gray-200 hover:bg-gray-100 hover:text-gray-700">
        ...
        <div className="grid gap-3">
          <div className="grid gap-2">
            <Label htmlFor="walletName" className="text-xs text-gray-700">Wallet Name/Alias (optional)</Label>
            <Input id="walletName" value={walletMetadata.name} onChange={e => handleWalletMetadataChange("name", e.target.value)} placeholder="e.g., My ETH Wallet, Hardware Wallet #1" className="satoshi-input placeholder:text-xs placeholder:text-muted-foreground/50" />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="walletDescription" className="text-xs text-gray-700">Notes/Description (optional)</Label>
            <Input id="walletDescription" value={walletMetadata.description} onChange={e => handleWalletMetadataChange("description", e.target.value)} placeholder="e.g., Ethereum main wallet, Cold storage" className="satoshi-input placeholder:text-xs placeholder:text-muted-foreground/50" />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {isCustomCount ? <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="icon" onClick={decreaseWordCount} disabled={wordCount <= 3}>
              <MinusCircle className="h-4 w-4" />
            </Button>
            <span className="w-8 text-center font-medium">{wordCount}</span>
            <Button type="button" variant="outline" size="icon" onClick={increaseWordCount} disabled={wordCount >= 24}>
          ...
          </div> : <Select onValueChange={handleWordCountChange} defaultValue="12">
      ...
      <div className="grid gap-4">
        <div className="flex justify-end">
          <Button type="button" variant="outline" size="sm" onClick={clearAllWords} className="text-gray-500 border-gray-200 hover:bg-gray-100 hover:text-gray-700">
            <Eraser className="h-3.5 w-3.5 mr-1.5" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {words.map((word, index) => <div key={index} className="flex items-center">
              <span className="mr-2 text-sm text-gray-500 w-6">{index + 1}:</span>
              <Input value={word} onChange={e => handleWordChange(index, e.target.value)} placeholder={`Word ${index + 1}`} className="satoshi-input placeholder:text-xs placeholder:text-muted-foreground/50" />
            </div>)}
        </div>
      </div>
    </div>;
